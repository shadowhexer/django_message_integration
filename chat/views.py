...
# The JavaScript fetch call expects a JSON response. Your original function was not providing this, leading to potential parsing errors or confusing behavior in the frontend, even when the data was successfully processed on the backend.
def create_message(request: HttpRequest) -> JsonResponse:
    if request.method == "POST":
        ...
        if not username:
            return JsonResponse({"error": "Unauthorized: No username in session"}, status=403)
